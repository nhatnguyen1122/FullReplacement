...
_PROVIDER_DEFAULTS: Dict[str, Dict[str, Any]] = {
    "codestral": {
        ...
        "api_key_env": ("CODESTRAL_API_KEY", "MISTRAL_API_KEY"),
        "model": "codestral-latest",
    },
    "mistral": {
        "api_base": "https://api.mistral.ai/v1",
        "api_key_env": ("MISTRAL_API_KEY", "CODESTRAL_API_KEY"),
        "model": "codestral-latest",
    },
    "gemini": {
...
def _get_provider_defaults(provider: Optional[str]) -> Dict[str, Any]:
    """Return provider defaults, accepting common aliases."""
    if not provider:
        return {}
    provider_key = provider.lower()
    return _PROVIDER_DEFAULTS.get(provider_key, {})


def _resolve_provider_api_key(provider: Optional[str]) -> Optional[str]:
    """Resolve the first available API key for a known provider."""
    defaults = _get_provider_defaults(provider)
    for env_name in defaults.get("api_key_env", ()):
...
def _apply_provider_defaults_to_dict(config_dict: Dict[str, Any]) -> Dict[str, Any]:
    """Apply provider defaults before dacite builds dataclasses."""
    ...
    defaults = _get_provider_defaults(provider)
    if defaults:
        llm.setdefault("api_base", defaults["api_base"])
        llm.setdefault("api_key", _resolve_provider_api_key(provider))
        has_legacy_models = bool(llm.get("primary_model") or llm.get("secondary_model"))
        if not llm.get("models") and not has_legacy_models:
            llm["models"] = [{"name": defaults["model"], "weight": 1.0, "provider": provider}]
        if not llm.get("evaluator_models") and not has_legacy_models:
            llm["evaluator_models"] = [
                {"name": defaults["model"], "weight": 1.0, "provider": provider}
            ]

    for models_key in ("models", "evaluator_models"):
        models = llm.get(models_key)
        if not models:
            continue
        normalized_models = []
        for model in models:
            ...
            model_defaults = _get_provider_defaults(model_provider)
            if model_defaults:
                model_dict.setdefault("api_base", model_defaults["api_base"])
                model_dict.setdefault("api_key", _resolve_provider_api_key(model_provider))
                model_dict.setdefault("name", model_defaults["model"])
            normalized_models.append(model_dict)
        llm[models_key] = normalized_models
...
class LLMModelConfig:
    ...
    def __post_init__(self):
        ...
        provider_defaults = _get_provider_defaults(self.provider)
        if provider_defaults:
            if self.api_base is None:
                self.api_base = provider_defaults["api_base"]
            if self.api_key is None:
                self.api_key = _resolve_provider_api_key(self.provider)
            if self.name is None:
                self.name = provider_defaults["model"]
...
class DatabaseConfig:
    ...
    num_islands: int = 5
    search_strategy: str = "island_map_elites"
...
@dataclass
class SearchConfig:
    """Configuration for search strategy (island MAP-Elites or TreeQD-MCTS)"""

    strategy: str = "tree_qd_mcts"  # Options: "tree_qd_mcts", "island_map_elites"

    # UCT parameters
    exploration_constant: float = 0.7  # c_explore in UCT formula
    qd_weight: float = 0.5  # weight on archive-potential term
    cost_weight: float = 0.1  # cost penalty weight
    max_tree_depth: int = 20

    # Backup strategy
    backup: str = "mixed"  # Options: "mean", "max", "mixed"
    backup_alpha: float = 0.5  # mix ratio for "mixed" backup (alpha * mean + (1-alpha) * max)

    # Progressive widening exponent
    widening_alpha: float = 0.5  # children allowed = ceil(visits ^ widening_alpha)
...
class Config:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "Config":
        config_dict = _apply_provider_defaults_to_dict(config_dict)
        ...
        config.database.search_strategy = config.search.strategy
...
def load_config(config_path: Optional[Union[str, Path]] = None) -> Config:
    ...
    if config_path and os.path.exists(config_path):
        ...
        # No-config default: Codestral through Mistral's OpenAI-compatible API.
        defaults = _get_provider_defaults("codestral")
        api_key = _resolve_provider_api_key("codestral")
        api_base = os.environ.get("CODESTRAL_API_BASE") or os.environ.get(
            "MISTRAL_API_BASE", defaults["api_base"]
        )
        config.llm.provider = "codestral"
