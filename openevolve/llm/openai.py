...
from openevolve.llm.token_budget import TokenBudgetExceeded, record_token_usage
...
def _usage_value(usage: Any, key: str) -> Optional[int]:
    """Extract a token usage field from pydantic objects or dictionaries."""
...
class OpenAILLM(LLMInterface):
    ...
    def __init__(
        ...
        self.track_token_usage = getattr(model_cfg, "track_token_usage", True)
    ...
    async def generate_with_context(
        ...
        # Skip seed parameter for endpoints that don't support it
        ...
        if seed is not None and not self.manual_mode:
            ...
            if self.api_base and any(ep in self.api_base for ep in skip_seed_endpoints):
                logger.debug(
                    "Skipping seed parameter as endpoint %s doesn't support it.",
                    self.api_base,
                )
            else:
                params["seed"] = seed
    ...
    def _record_response_usage(self, response: Any) -> None:
        """Record token usage from OpenAI-compatible responses when available."""
