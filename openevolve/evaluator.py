...
class Evaluator:
    ...
    def _process_evaluation_result(self, result: Any) -> EvaluationResult:
        ...
        if isinstance(result, dict):
            if isinstance(result.get("metrics"), dict):
                ...
                return EvaluationResult(metrics=result["metrics"], artifacts=artifacts)
    ...
    async def _direct_evaluate(
        ...
        # Run direct evaluation in a subprocess so generated program timeouts
        # can be killed instead of leaving stuck threads in worker processes.
