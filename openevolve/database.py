...
@dataclass
class AddResult:
    """Insertion metadata used by TreeQD reward computation."""
...
def _normalise_evaluation_metrics(metrics: Dict[str, Any]) -> Dict[str, Any]:
    """Support both flat metrics and nested evaluator return shapes."""
...
def _evaluation_succeeded(metrics: Dict[str, Any]) -> bool:
    """Infer whether evaluation produced a usable candidate program."""
    if not metrics:
        return False

    metrics = _normalise_evaluation_metrics(metrics)

    if metrics.get("timeout") is True:
        return False

    metric_status = str(metrics.get("evaluation_status", "")).upper()
    if metric_status in {"ERROR", "VALIDATION_ERROR", "EXCEPTION", "TIMEOUT"}:
        return False

    artifacts = metrics.get("artifacts")
    if isinstance(artifacts, dict):
        status = str(artifacts.get("status", "")).upper()
        if status in {"ERROR", "VALIDATION_ERROR", "EXCEPTION", "TIMEOUT"}:
            return False
    ...
    for key in ("runs_successfully", "validity"):
...
class ProgramDatabase:
    ...
    def __init__(self, config: DatabaseConfig):
        ...
        # Tree-MCTS population metadata. These are active only when
        # config.search_strategy == "tree_qd_mcts"; they do not gate admission
        # like a MAP-Elites grid.
        self.tree_programs: Set[str] = set()
        self.tree_feature_counts: Dict[str, int] = {}
    ...
    def add(
        ...
        Returns:
            Program id for backward compatibility with existing OpenEvolve callers
        """
        return self.add_with_result(program, iteration=iteration, target_island=target_island).program_id

    def add_with_result(
        self, program: Program, iteration: int = None, target_island: Optional[int] = None
    ) -> "AddResult":
        """
        Add a program to the database and return insertion metadata.

        TreeQD-MCTS uses this metadata to compute rewards without changing the
        long-standing ProgramDatabase.add() return type.
        """
        # Prepare AddResult tracking
        add_result = AddResult(program_id=program.id)

        # Compute parent fitness for delta reward
        if program.parent_id and program.parent_id in self.programs:
            parent_prog = self.programs[program.parent_id]
            add_result.parent_fitness = get_fitness_score(
                parent_prog.metrics, self.config.feature_dimensions
            )
        ...
        previous_program = self.programs.get(program.id)
        self.programs[program.id] = program

        if getattr(self.config, "search_strategy", "tree_qd_mcts") == "tree_qd_mcts":
            return self._add_with_result_tree_only(
                program=program,
        ...
        add_result.island_idx = island_idx
        add_result.fitness = get_fitness_score(program.metrics, self.config.feature_dimensions)
        add_result.evaluation_success = _evaluation_succeeded(program.metrics)

        if not add_result.evaluation_success:
            logger.debug(
                "Program %s failed evaluation and won't be added to island %d",
                program.id,
                island_idx,
            )
            if previous_program is None:
                ...
                self.programs[program.id] = previous_program
            add_result.accepted = False
            return add_result

        # Calculate feature coordinates for MAP-Elites only after the candidate is usable.
        ...
        if not self._is_novel(program.id, island_idx):
            ...
            add_result.accepted = False
            add_result.island_idx = island_idx
            return add_result  # Do not add non-novel program
        ...
        if should_replace:
            ...
            if feature_key not in island_feature_map:
                ...
                add_result.added_to_new_cell = True
                logger.info(
                ...
                existing_program_id = island_feature_map[feature_key]
                add_result.improved_existing_cell = True
                add_result.replaced_program_id = existing_program_id
        ...
        # Finalize AddResult
        add_result.island_idx = island_idx
        add_result.feature_key = feature_key
        add_result.fitness = get_fitness_score(program.metrics, self.config.feature_dimensions)
        add_result.evaluation_success = _evaluation_succeeded(program.metrics)

        return add_result

    def _add_with_result_tree_only(
        self,
        program: Program,
        add_result: "AddResult",
        previous_program: Optional[Program] = None,
    ) -> "AddResult":
        """Add a program under the MCTS tree-only population policy.

        This path deliberately bypasses island-local MAP-Elites replacement.
        The old island fields are maintained only as compatibility metadata for
        prompt construction and checkpoint consumers.
        """
        island_idx = 0
        if not self.islands:
            self.islands = [set()]
        if len(self.islands) > 1:
            # Keep old structures allocated for legacy code, but the active
            # MCTS population lives in bucket 0.
            island_idx = 0
        ...
        if not add_result.evaluation_success:
            logger.debug("Program %s failed evaluation and won't enter the MCTS tree", program.id)
            if previous_program is None:
                self.programs.pop(program.id, None)
            else:
                self.programs[program.id] = previous_program
        ...
        if not self._is_novel(program.id, island_idx):
            logger.debug("Program %s failed novelty check and won't enter the MCTS tree", program.id)
            if previous_program is None:
                self.programs.pop(program.id, None)
            else:
                self.programs[program.id] = previous_program
        ...
        feature_key = self._feature_coords_to_key(self._calculate_feature_coords(program))
        add_result.feature_key = feature_key
        add_result.added_to_new_cell = self.tree_feature_counts.get(feature_key, 0) == 0
        add_result.improved_existing_cell = add_result.fitness > add_result.parent_fitness
        self.tree_feature_counts[feature_key] = self.tree_feature_counts.get(feature_key, 0) + 1

        self.tree_programs.add(program.id)
        self.islands[island_idx].add(program.id)
        program.metadata["island"] = island_idx
        program.metadata["search_strategy"] = "tree_qd_mcts"
        program.metadata["tree_feature_key"] = feature_key
        ...
        logger.debug("Added program %s to MCTS tree population", program.id)
        return add_result

    def sample_inspirations_for_parent(
        self, parent: Program, num_inspirations: Optional[int] = None
    ) -> List[Program]:
        """Sample inspirations using the exact selected parent."""
        if num_inspirations is None:
            num_inspirations = 5
        return self._sample_inspirations(parent, n=num_inspirations)
    ...
    def save(self, path: Optional[str] = None, iteration: int = 0) -> None:
        ...
        metadata = {
            ...
            "tree_programs": list(self.tree_programs),
            "tree_feature_counts": self.tree_feature_counts,
    ...
    def load(self, path: str) -> None:
        ...
        if os.path.exists(metadata_path):
            ...
            self.tree_programs = set(metadata.get("tree_programs", []))
            self.tree_feature_counts = dict(metadata.get("tree_feature_counts", {}))
        ...
        self.tree_programs = {pid for pid in self.tree_programs if pid in self.programs}
        if getattr(self.config, "search_strategy", "tree_qd_mcts") == "tree_qd_mcts":
            if not self.tree_programs:
                self.tree_programs = set(self.programs.keys())
            for program_id in self.tree_programs:
                if program_id in self.programs:
                    self.programs[program_id].metadata.setdefault("island", 0)
                    if self.islands:
                        self.islands[0].add(program_id)
    ...
    def _enforce_population_limit(self, exclude_program_id: Optional[str] = None) -> None:
        ...
        for program in programs_to_remove:
            ...
            self.tree_programs.discard(program_id)
