...
from openevolve.llm.token_budget import configure_token_budget, token_usage_to_dict
...
class OpenEvolve:
    ...
    def __init__(
        ...
        self.config = config
        self.config.database.search_strategy = self.config.search.strategy
        ...
        self.convergence_log_path = os.path.join(self.output_dir, "convergence.jsonl")

        # Set up logging
        self._setup_logging()

        if self.config.llm.track_token_usage:
            configure_token_budget(max_total_tokens=self.config.llm.max_total_tokens)
            if self.config.llm.max_total_tokens is not None:
                logger.info(
                    "Token budget enabled: max_total_tokens=%s",
                    self.config.llm.max_total_tokens,
                )
        ...
        # Initialize TreeQD-MCTS controller if configured
        self.tree_controller = None
        if self.config.search.strategy == "tree_qd_mcts":
            from openevolve.search.tree_qd import TreeQDController
            self.tree_controller = TreeQDController.from_config(self.config.search)
            logger.info("TreeQD-MCTS controller enabled")
    ...
    async def run(
        ...
        if should_add_initial:
            ...
            initial_add_result = self.database.add_with_result(initial_program)
            self._append_convergence_record(
                {
                    ...
                    "score": get_fitness_score(
                        initial_metrics, self.config.database.feature_dimensions
                    ),
                    "primary_metric": get_primary_metric_name(initial_metrics),
                    "accepted": initial_add_result.accepted,
                    "evaluation_success": initial_add_result.evaluation_success,
                    "fitness": initial_add_result.fitness,
                    "parent_fitness": initial_add_result.parent_fitness,
                    "added_to_new_cell": initial_add_result.added_to_new_cell,
                    "improved_existing_cell": initial_add_result.improved_existing_cell,
                    "replaced_program_id": initial_add_result.replaced_program_id,
                    "generation": initial_program.generation,
                    "island": initial_program.metadata.get("island"),
                    "target_island": None,
                    "metrics": initial_metrics,
                    "iteration_time": None,
                    ...
                    "error": initial_metrics.get("error")
                    if isinstance(initial_metrics, dict)
                    else None,
                }
            )

            # Register initial program in tree controller
            if self.tree_controller is not None and initial_add_result.accepted:
                self.tree_controller.register_program(
                    program_id=initial_program_id,
                    iteration=start_iteration,
                    feature_key=initial_add_result.feature_key,
                    fitness=get_fitness_score(
                        initial_metrics, self.config.database.feature_dimensions
                    ),
                )

            # Check if an explicit primary fitness metric is present.
            if get_primary_metric_name(initial_metrics) is None:
        ...
        self._register_database_programs_in_tree()
        ...
        try:
            self.parallel_controller = ProcessParallelController(
                ...
                convergence_log_path=self.convergence_log_path,
            )
            ...
            if self.config.llm.track_token_usage:
                usage = token_usage_to_dict()
                logger.info(
                    "Final token usage: total=%s prompt=%s completion=%s requests=%s",
    ...
    def _append_convergence_record(self, record: Dict[str, Any]) -> None:
        ...
        with open(self.convergence_log_path, "a", encoding="utf-8") as handle:
    ...
    def _save_checkpoint(self, iteration: int) -> None:
        ...
        if best_program:
            ...
            logger.warning("No best program available to save at checkpoint %s", iteration)

        # Save tree controller state if active
        if self.tree_controller is not None:
            tree_state_path = os.path.join(checkpoint_path, "tree_qd_state.json")
            self.tree_controller.save(tree_state_path)
    ...
    def _load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        # Load tree controller state if present
        if self.tree_controller is not None:
            tree_state_path = os.path.join(checkpoint_path, "tree_qd_state.json")
            if os.path.exists(tree_state_path):
                from openevolve.search.tree_qd import TreeQDController
                self.tree_controller = TreeQDController.load(tree_state_path)
                logger.info("Loaded TreeQD-MCTS state from checkpoint")
            else:
                logger.info("No TreeQD-MCTS state found in checkpoint; rebuilding from database")
            self._register_database_programs_in_tree()

        logger.info(f"Checkpoint loaded successfully (iteration {self.database.last_iteration})")

    def _register_database_programs_in_tree(self) -> None:
        """Ensure existing database programs are represented in the TreeQD tree."""
        ...
        programs = sorted(
            ...
            key=lambda p: (p.generation, p.iteration_found, p.timestamp),
        )
        registered = 0
        for program in programs:
            if program.id in self.tree_controller.program_to_node:
                continue
            feature_key = self.database._feature_coords_to_key(
                self.database._calculate_feature_coords(program)
            )
            self.tree_controller.register_program(
                ...
                fitness=get_fitness_score(program.metrics, self.config.database.feature_dimensions),
            )
            registered += 1

        if registered:
            logger.info("Registered %d database programs in TreeQD-MCTS tree", registered)
    ...
    async def _run_evolution_with_checkpoints(
        ...
        if self.config.search.strategy == "tree_qd_mcts":
            logger.info("Using TreeQD-MCTS tree-based evolution")
        else:
            logger.info(
