...
"""
    ...
    def test_record_tree_error_backpropagates_failed_rollout(self):
        """Failed LLM/parser iterations should count as negative MCTS rollouts."""
        tree_controller = TreeQDController.from_config(self.config.search)
        node = tree_controller.register_program("test_0", fitness=0.5)
        ...
        controller._pending_tree_nodes[7] = node.node_id

        controller._record_tree_error(7, "No valid diffs found in response")

        self.assertEqual(node.pending_count, 0)
        self.assertEqual(tree_controller.root.visits, 1)
        self.assertEqual(node.visits, 1)
        self.assertAlmostEqual(node.value_sum, -self.config.search.w_invalid)
        self.assertNotIn(7, controller._pending_tree_nodes)
