...
## Server Experiment: OpenEvolve Baseline Matrix
...
Each benchmark runs 5 independent runs, 300 iterations per run. The run script writes per-run configs and outputs under `output/`, which is ignored by git. Do not commit API keys, `.env` files, logs, result databases, or output directories.

### Linux Conda Setup

```bash
git clone <FULL_REPLACEMENT_MCTS_REPO_URL> FullReplacementMCTS
cd FullReplacementMCTS

conda create -n openevolve_exp python=3.11 -y
conda activate openevolve_exp
...
Set keys in the shell or a local `.env` file that is never committed:
...
python scripts/run_benchmark_matrix.py \
  ...
  --output-root output/server_baseline_sp_cp26_cp32_cprect_erdos_5runs_300iter_20mtok \
  --resume
```
...
- Results summary is written under the selected `--output-root`; each run also has its own log, generated config, checkpoints, and best program.
