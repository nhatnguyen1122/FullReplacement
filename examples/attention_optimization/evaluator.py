...
class MLIRAttentionEvaluator:
    def __init__(self):
        self.verify_tools()
        self.mlir_file = Path(__file__).resolve().parent / "mlir" / "self_attn_with_consts_linalg_dialect.mlir"
        # self.mlir_file = Path("mlir/export_mlir.mlir")
        self.baseline_mlir = None
        self.baseline_metrics = None

    def stable_seed(self, value):
        """Create a deterministic seed from program text or parameter content."""
        if not isinstance(value, str):
            value = json.dumps(value, sort_keys=True, default=str)
        digest = hashlib.sha256(value.encode("utf-8")).digest()
        return int.from_bytes(digest[:8], "big")

    def call_with_stable_random(self, func, seed):
        """Run stochastic candidate code with a seed derived from its source."""
    ...
    def evaluate(self, optimize_attention_input):
        ...
        try:
            ...
            if isinstance(optimize_attention_input, str):
                if os.path.isfile(optimize_attention_input) and optimize_attention_input.endswith('.py'):
                    ...
                    if 'optimize_attention' in namespace:
                        ...
                        params = self.call_with_stable_random(
                            optimize_attention_func,
                            self.stable_seed(code),
                        )
                    else:
                ...
                params = self.call_with_stable_random(
                    optimize_attention_input,
                    self.stable_seed(repr(optimize_attention_input)),
                )
            ...
            print(f"Optimized MLIR generated ({len(optimized_mlir)} chars)")
            ...
            combined_score = max(0.0, speedup / max(target_speedup, 1e-9))
